...
export const facultiesResolver = {
  Query: {
    faculties: async (_, args) => {
  ...
  Faculty: {
    classes: async (parent: Faculty) => {
      return await prisma.class.findMany({ where: { facultyId: parent.id } });
    },
    trainingFields: async (parent: Faculty) => {
      return await prisma.trainingField.findMany({
        where: { facultyId: parent.id },
      });
    },
  },
};
